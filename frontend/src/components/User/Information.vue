<template>
    <div class="container view">
        <div class="left_box">
            <ul class="aside">
                <li v-for="item in asideList" :key="item.id" :class="{asideActive: item.id === asideIndex}"
        ...
        <div class="view-container" style="margin-left: 250px">
            ...
            <div v-show="asideIndex === 1" class="cont view">
                <div class="right_cont">
                    ...
                    <div class="user_info">
                        <div class="header">
                            <img src="@/assets/images/logo.jpg" alt="logo" class="head">
                            <!--                            <p class="modify">修改头像</p>-->
                        </div>
                        <div class="right_c">
                            <div class="nick">
								<span class="mod" @click="changeInformationFlag = true" v-if="!changeInformationFlag">
									修改资料
								</span>
                            ...
                            <ul class="self">
                                <li class="comon" v-for="item in userInfo" :key="item.id">
                                    <div>
            ...
            <div v-show="asideIndex === 2" class="acc_project">
                ...
                <div class="acc_pass_con">
                    ...
                    <el-table ref="multipleTable" :data="fileTableData" tooltip-effect="dark" style="width: 100%"
...
<script>
    ...
    export default {
        ...
        created() {
            // 1.获取用户个人资料
            userApi.information({username: localStorage.getItem('username')}).then(response => {
                this.userInfo[1].value = response.data.name;
                this.userInfo[2].value = response.data.gender === 2 ? '男' : '女';
                this.userInfo[3].value = response.data.phone;
                this.userInfo[4].value = response.data.mailbox;
            }).catch(error => {
                this.$message({
                    message: error,
                    type: 'error',
            ...
            userApi.file({username: localStorage.getItem('username')}).then(response => {
                this.userLevel = response.data.userlvl;
                this.userTotalSize = response.data.totalsize;
                this.userFileCount = response.data.filecount;
            }).catch(error => {
                this.$message({
                    message: error,
                    type: 'error',
        ...
        methods: {
            handleDelete(index, row) {
                projectApi.deleteFile({id: row.id}).then(res => {
                    Notification({
                        title: '成功',
                        message: '文件移除成功',
                        type: 'success',
            ...
            changePassword() {
                ...
                userApi.changePassword(changePasswordParam).then(_ => {
                    this.$message('密码更改成功！');
                }).catch(err => {
                    this.$message({
                        message: err,
            ...
            saveChangeInformation() {
                ...
                // eslint-disable-next-line no-unused-vars
                userApi.changeInformation(changeData).then(response => {
                    this.$message({
                        message: '保存成功！',
                        type: 'success',
                    });
